...
function Keypad({ board, onNumber, onErase }: KeypadProps) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-1.5 rounded-2xl border border-grid/10 bg-surface p-2 shadow-toolbar">
      {Array.from({ length: 9 }, (_, i) => i + 1).map((n) => {
        ...
        return (
          <button
            ...
            aria-label={`Enter ${n}`}
            className={[
              'flex h-11 w-11 items-center justify-center rounded-xl text-lg font-medium transition-all sm:h-12 sm:w-12',
              isComplete
      ...
      <button
        ...
        className="flex h-11 items-center justify-center rounded-xl bg-paper px-3.5 text-sm font-medium text-ink/70 shadow-sm transition-all hover:bg-accentSoft active:scale-95 sm:h-12"
      >
        Erase
      </button>
    </div>
